...
PROD_REPO=${CLOUDSMITH_PROD_REPO:-'opencpn/statusbar-prod'}
BETA_REPO=${CLOUDSMITH_BETA_REPO:-'opencpn/statusbar-beta'}
ALPHA_REPO=${CLOUDSMITH_ALPHA_REPO:-'opencpn/statusbar-alpha'}
...
if [ -n "${OCPN_TARGET}" ]; then
    tarball_name=statusbar_pi-1.1.12.0-${PKG_TARGET}-x86_64-wx-${PKG_TARGET_VERSION}-${OCPN_TARGET}-tarball
else
    tarball_name=statusbar_pi-1.1.12.0-${PKG_TARGET}-x86_64-wx-${PKG_TARGET_VERSION}-tarball
fi
...
if [ "$BUILD_BRANCH_LOWER" = "master" ]; then
    ...
    if [ -n "$BUILD_TAG"  ]; then
        ...
        VERSION="1.1.12.0+${BUILD_ID}.${commit}"
...
# Only upload if the owner did the pull request. Non-owners probably don't have security to upload to the cloudsmith repository
    if [ "$CIRCLE_PROJECT_USERNAME" = "$CIRCLE_USERNAME" ]; then
        cloudsmith push raw --republish --no-wait-for-sync \
            --name statusbar_pi-1.1.12.0-msvc-x86_64-wx-10.0.26200-${OCPN_TARGET}-metadata \
            --version ${VERSION} \
            --summary "statusbar opencpn plugin metadata for automatic installation" \
            $REPO $xml

        cloudsmith push raw --republish --no-wait-for-sync \
            --name $tarball_name \
            --version ${VERSION} \
            --summary "statusbar opencpn plugin tarball for automatic installation" \
            $REPO $tarball

        if [ "${PKG_EXT}" != "gz" ] && [ "$pkg" != "" ]; then
            cloudsmith push raw --republish --no-wait-for-sync \
                --name opencpn-package-statusbar-1.1.12.0-msvc-x86_64-wx-10.0.26200-${OCPN_TARGET}.${PKG_EXT} \
                --version ${VERSION} \
                --summary "statusbar .${PKG_EXT} installation package" \
                $REPO $pkg
        fi
    fi
fi
